...
//        }
        ...
        HashMap<String, Set<String>> mappa = new HashMap<>();
        Set<String> toAdd = new HashSet<>();
        String[] add = {"a", "b", "c"};
        String[] add1 = {"d", "e"};
        toAdd.addAll(Arrays.asList(add));
        mappa.put("d", toAdd);
        toAdd.addAll(Arrays.asList(add1));
        mappa.put("f", toAdd);
        Set<String> toAdd1 = new HashSet<>();
        toAdd1.addAll(Arrays.asList(add1));
        mappa.put("a", toAdd1);
        for (Map.Entry<String, Set<String>> entry: mappa.entrySet())
            System.out.println(entry);
        for (String toprint: MicroBlog.influencers(mappa)){
            System.out.println(toprint);
        }
        try {
            MicroBlog.addPost("Gigio", "Cioa, asodaosp, as,d,paso, ' as; sa' ?sad ");

        }catch (IllegalLengthException | EmptyTextException | NullPointerException e){
            System.out.println(e);
        }
        MicroBlog.printAllPosts();
        MicroBlog.reportPostsByWord("cioa");
        System.out.println("Segnalato");
        MicroBlog.printAllPosts();
        try {
            MicroBlog.addPost("Ezio", "Cioa, asodaosp, as,d,paso, ' as; sa' ?sad ");
